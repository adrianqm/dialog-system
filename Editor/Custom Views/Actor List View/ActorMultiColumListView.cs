...
public class ActorMultiColumListView : MultiColumnListView
{
    ...
    {
        if (database.actors == null) return;

        _database = database;
        _currentActorList = database.actors;
    ...
    private void FocusOnLastElement()
    {
        ...
        int lastIndex = _database.actors.Count - 1;
        ...
        var nameField = itemView.Q<TextField>(className:"actorsTextFieldCell--textField");
        nameField.Focus();
    }
    ...
    private void RemoveSelectedActors()
    {
        ...
        if (deleteClicked)
        {
            List<Actor> auxArray = new List<Actor>(_database.actors);
